...
def AvailableTrains():
    ...
    mn = mysql.connector.connect(host="localhost", user="root",
                                 password="admin@123", database="railway")
    cur = mn.cursor()
    ...
    cur.execute(
        'SELECT Train_No, Source_Station_Name, Destination_Station_Name, Arrival_Time, Departure_Time from train_info where Source_Station_Code=%s AND Destination_Station_Code=%s;',
        (start_opt, final_opt))
    result = cur.fetchall()
...
def CheckFare():
    ...
    cur.execute(
        'SELECT Train_No, Distance FROM train_info WHERE Source_Station_Code=%s AND Destination_Station_Code=%s;',
        (start_opt, final_opt)
    )
    ...
    if len(result_fare) >= 10:
        try:
            print(f"Total of {len(result_fare)} Records Found!")
            ask = int(input("Enter the Number of Records you want to See: "))
        except ValueError:
            print("Please Enter a Valid Integer!")
        else:
            ...
            for x in range(ask):
                y = result_fare[x]
                print(y, "Rs.", int(y[1]) * sleeper_charge, "Rs.", int(y[1]) * third_ac_charge, "Rs.",
...
def ShowBookings():
    ...
    cur.execute('SELECT * FROM bookings WHERE Mobile_No=%s', (mobile_no,))
    ...
    if len(result) == 0:
        ...
        for x in result:
            print(f"BOOKING NO {booking_no}:", x, "\n")
            booking_no += 1
...
def BookTrain():
    ...
    while True:
        ...
        if ask in ["Y", "y"]:
            print("Booking...")
            try:
                query = "INSERT INTO bookings (Train_No, Passenger_Name, Mobile_No, Passenger_Adhaar, Time_Of_Booking, Booking_ID, Class) VALUES (%s, %s, %s, %s, %s, %s, %s)"
                cur.execute(query, (train_no, Name, Mobile, adhaar, date, id, Class))
            except DataError:
    ...
    while True:
        try:
            ...
            if len(str(unique_id)) == 0:
                ...
                cur.execute("SELECT * FROM bookings WHERE Booking_ID=%s", (unique_id,))
                result = cur.fetchall()
                if len(result) == 0:
                    print("No Records Found!")
                    break
                ...
                while True:
                    ...
                    if ask in ["Y", "y"]:
                        cur.execute("DELETE FROM bookings WHERE Booking_ID=%s", (unique_id,))
                        print("Deleted!")
                        mn.commit()
                        cur.close()
                        mn.close()
